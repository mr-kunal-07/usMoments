...
export default function JoinPage() {
  ...
  useEffect(() => {
    ...
    if (!user) {
      ...
      ephemeralStorage.set(JOIN_REDIRECT_KEY, APP_PATHS.join);
  ...
  if (status === "done" || couple?.status === "active") {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-6 bg-background px-6 text-center">
        ...
        <Button onClick={() => navigate(APP_PATHS.dashboard)} className="w-full max-w-xs gap-2">
  ...
  if (status === "error") {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-6 bg-background px-6 text-center">
        ...
        <Button variant="outline" onClick={() => navigate(APP_PATHS.profile)}>Go to Profile</Button>
      </div>
    );
  }
